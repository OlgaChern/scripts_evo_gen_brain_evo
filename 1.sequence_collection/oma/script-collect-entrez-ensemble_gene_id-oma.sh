...
#then
#	rm $file_out
#	rm $file_out-ALL-INFO
#fi

#if [ -e $file_entrez_human_brain-oma_id-NF ]
#then
#	rm $file_entrez_human_brain-oma_id-NF
#fi
...
m="`wc -l $file_entrez_human_brain | awk -F " " '{print $1}'`"
g=1
cat $file_entrez_human_brain | while read -r line
do
	echo "ENTREZ $g out of $m: $line"
	entrez="$line"
	g=$[$g+1]

	echo "getting oma_ids.."
	genes_num="`grep "=$line=" ${file_oma_entrez}-MOD |wc -l | awk -F " " '{print $1}'`"


if [ $genes_num -gt 0 ]
then

	oma_ids="`grep "=$line=" ${file_oma_entrez}-MOD | awk -F "=" '{print $1}' |  xargs -n $genes_num`"
 ...
 do 
	...
	echo "|$entrez|$oma_id|$ensembl_info" >> $file_out
	...
	then
		...
		echo "|$entrez|$oma_id|$ensembl_info|OMAgr$oma_gr|sp_NUM_$oma_gr_sp_num|$oma_gr_sp" >> $file_out-ALL-INFO	
 done

else
	#echo "NOT_FOUND oma_id for $g entrez: $entrez"
	echo "$entrez" >> $file_entrez_human_brain-oma_id-NF
fi
